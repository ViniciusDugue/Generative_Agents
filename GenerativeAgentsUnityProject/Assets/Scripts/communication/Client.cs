...
public class Client : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (inputField != null)
        {
            ...
            Debug.Log("[Client] InputField listener attached.");
        }
        else
        {
            Debug.LogWarning("[Client] No InputField assigned.");
        }

        BehaviorManager[] agents = GameObject.FindObjectsByType<BehaviorManager>(FindObjectsSortMode.None);
        foreach (BehaviorManager bm in agents)
        {
            RegisterAgent(bm.gameObject);
            Debug.Log("Registered Agent: " + bm.gameObject.name);
        }
    }
